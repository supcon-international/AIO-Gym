...
- **[チャレンジモード](challenge.html)**：モバイル+デスクトップのミニゲーム、4設備から選択（反応器 / 空調 / タンク連結 / 加熱炉）——RL ゴーストと**並んで**同じ外乱で競い（2つのライブ P&ID）、経済スコアを競う。放置では勝てないスコア設計。
...
| **管式加熱炉** | 製油所の加熱炉:燃料 + ダンパー | 最小燃料で出口 370°C を維持——過剰空気は熱を奪い、空気不足は低 O₂ で燃料遮断 |
...
同じ 5 つのプラントはネイティブの **Gymnasium** 環境でもあり、ブラウザなしで学習できます：
